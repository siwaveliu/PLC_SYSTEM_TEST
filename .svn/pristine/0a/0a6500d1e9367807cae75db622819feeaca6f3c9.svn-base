...
价模块
19. 经过 2 个路由周期，测试用例通过载波透明接入单元发送四级站点抄表报文
20. 载波信道侦听单元收到被测 sta 的路由请求报文后，上传给测试台体，再送给一致性评
价模块，判断被测 sta 的路由请求报文正确后，通知测试用例。
21. 等待一段时间后，被测 sta 发送路由错误报文给二级站点。
22. 载波信道侦听单元收到被测 sta 的路由错误报文后，上传给测试台体，再送给一致性评
价模块
'''
def run(tb):
    """
    ...
    tb.mac_head.org_dst_tei         = 6  #DUT tei is 3, trig packet routing request
    tb.mac_head.org_src_tei         = 1
    tb.mac_head.msdu_type           = "PLC_MSDU_TYPE_APP"
    tb.mac_head.tx_type             = 'PLC_MAC_UNICAST'
    tb.mac_head.mac_addr_flag       = 0
    tb.mac_head.hop_limit           = 10
    tb.mac_head.remaining_hop_count = 5
    tb.mac_head.broadcast_dir       = 1 #downlink broadcast

    tb._send_msdu(msdu=msdu, mac_head=tb.mac_head, src_tei=3, dst_tei=4, broadcast_flag = 0)
...
#     tb.mac_head.broadcast_dir       = 1 #downlink broadcast
#     tb._send_msdu(msdu=msdu, mac_head=tb.mac_head, src_tei=3, dst_tei=4, broadcast_flag = 1)
    ...
    assert mac_frame_head.org_dst_tei == 6
    ...
    time.sleep(2)
    [fc, mac_frame_head, nmm] = tb._wait_for_plc_nmm('MME_ROUTE_ERROR',10)
