...
'''
1. 待测STA与透明物理转发设备处于隔变环境。
2. 软件平台(电能表)模拟电表，在收到待测STA请求读表号后，向其下发电表地址信息()。
3. 软件平台(CCO)模拟CCO功能，通过透明物理转发设备，处理待测STA的入网请求，分配TEI=2给待测STA，确保被测STA成功入网。
...
6. 软件平台(CCO)产生事件报文下行帧（缓存区满），通过透明物理转发设备将该报文转发至待测STA
7. 在120s内，软件平台(CCO)若收到待测STA重发的事件报文上行帧，且格式满足要求，则测试通过，否则测试不通过
check:
待测STA是否在规定时间内重发事件报文上行帧，且格式满足：
1. 两帧事件报文上行帧中的“方向位”是否均为1
2. 两帧事件报文上行帧中的“启动位”是否均为1
3. 两帧事件报文上行帧中的“功能码”是否均为1
5. 两帧事件报文上行帧中的“电能表地址”是否与分配的表地址一致
6. 两帧事件报文上行帧中的“协议版本号”是否为1
'''
def run(tb):
    """
    Args:
    ...
    # 6. 软件平台(CCO)产生事件报文下行帧（缓存区满），通过透明物理转发设备将该报文转发至待测STA
    plc_tb_ctrl._debug("Step 6: simulate CCO to ack STA APL event pkt, with buffer full code...")
    ...
    dl_event_report_pkt['body']['op_code']    = 'EVENT_CODE_EVENT_BUFF_FULL'
    ...
    # 7. 在120s内，软件平台(CCO)若收到待测STA重发的事件报文上行帧，且格式满足要求，则测试通过，否则测试不通过
    plc_tb_ctrl._debug("Step 7: simulate CCO to receive the APL event pkt re-sent by STA, and do the pkt valid check...")

    [timestamp, fc, mac_frame_head, apm] = tb._wait_for_plc_apm_ul('APL_EVENT_REPORT',120)
