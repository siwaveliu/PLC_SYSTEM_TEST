...
def check(tdr, tb, test_data):
    ...
    while succ_rate < TARGET_SUCC_RATE:
        ...
        #plc_tb_ctrl._trace_byte_stream("rx_debug", frame)
        total_cnt += 1
        ...
        if succ_rate < TARGET_SUCC_RATE :
            if total_cnt = packet_num : #fail at current fadding value
                plc_tb_ctrl._debug("Fadding: {}, correct: {}, succ_rate:{}".format(current_fad, corr_cnt, succ_rate))
                self.current_fad = 20
                fadding.set_fadding(current_fad)
                break
            #else : continue on testing
        else : #current fadding value passed
            corr_cnt = 0
            total_cnt = 0
            succ_rate = float(corr_cnt) / packet_num
            self.current_fad += 1
...
def run(tb):
    ...
    # 设置信号发生器
	self.siggen_class = siggen.SigGen_Keysight(config.SIGGEN_ADDR)
    siggen_class.awgn_on(NBW,CBW)
    siggen_class.noise_atten(30)
