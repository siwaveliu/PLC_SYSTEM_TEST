...
band0_test_data_set = [
    {'band': 0, 'mpdu_type': plc_packet_helper.MAC_MPDU_SACK, 'tmi_b': 0, 'tmi_e': 0},
...
def run(tb):
    ...
    siggen_plc.reset()
    siggen_plc.awgn_on(25,25)
    siggen_plc.noise_power(-10)
    siggen_plc.noise_atten(0)
    ...
    for test_data in band0_test_data_set:
        result_file.write("*************************************\n")
        result_file.write("band {:d} mpdu_type {:d} tmi_b {:d} tmi_e {:d} \n".format(band,int(test_data['mpdu_type']),int(test_data['tmi_b']),int(test_data['tmi_e'])))
