...
价模块。
11. 测试用例按照信标周期通过载波透明接入单元发送中央信标报文，不发送发现列表报
文，超过连续 4 个路由周期后。
...
def run(tb):
    ...
    #get current time after beacon configured immediately
    cur_time = time.time()
...
#     tb._configure_beacon(None, beacon_dict,True)
    ...
    while(time.time() < cur_time + tc_common.calc_timeout(route_period*4)):
