...
'''
3.2.4.3 长 MPDU 帧载荷长度 264 长 MAC 帧头的 SOF 帧是否能够被正确处理测试
验证长 mpdu 帧载荷长度 264 长 mac 帧头的 sof 帧是否能够被正确处理
'''
def run(tb):
    """
    run function.
    ...
    pb_num_per_mpdu = 4
    total_pb_num = 4
    msdu_len = plc_packet_helper.calc_max_msdu_len(pb_size, 4, False)
