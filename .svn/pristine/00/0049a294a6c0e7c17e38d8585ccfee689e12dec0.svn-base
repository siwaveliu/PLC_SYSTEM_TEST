# -- coding: utf-8 --
# STA 通过集中器主动并发抄表测试（单个STA 抄读多个数据项的645 帧）
import robot
import plc_tb_ctrl
import concentrator
import tc_common
...
'''
1. 连接设备，上电初始化。
2. 软件平台模拟电表，在收到被测STA 请求读表号后，向其下发电表地址信息。
3. 软件模拟CCO 对入网请求的STA 进行处理，确定站点入网成功，完成组网。
4. 软件模拟CCO 向被测STA 启动集中器主动并发抄表业务，发送SOF 帧（抄表报文下行），
   用于抄读 STA 所在设备（DL/T645-2007 规约虚拟电表000000000001）的多个特定数据项
   (读当前正向有功总电能、读日期和星期、读时间)。
5. 被测STA 收到下行抄表报文后分别读取下挂电能表的当前正向有功总电能、日期和星期、时间，
   待测STA 以规定的时间频率与电能表之间进行交互。
6. 软件平台模拟电能表向被测STA 分别返回抄读数据项，待测STA 组织S0F 帧（抄表报文上行）回复CCO。
7. 在虚拟电表的TTL 串口监控是否收STA 转发的数据报文，如在n 秒内未收到，则指示STA 抄表下行转发失败。
   如在n 秒内收到，则指示STA 下行转发数据成功，虚拟电表针对数据报文进行解析并应答电表当前报文。
...
3. 测试STA 转发上行抄表数据时其报文ID 是否为0x0003（集中器主动并发抄表）；
...
def run(tb):
    ...
    # 4. 软件模拟CCO 向被测STA 启动集中器主动并发抄表业务，发送SOF 帧（抄表报文下行），
    #    用于抄读 STA 所在设备（DL/T645-2007 规约虚拟电表000000000001）的多个特定数据项(读当前正向有功总电能、
    #    读日期和星期、读时间)。
