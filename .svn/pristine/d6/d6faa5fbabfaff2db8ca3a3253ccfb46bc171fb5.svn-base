...
def check(tdr, tb, test_data,result_file,siggen_plc):
    ...
    if test_narrow:
        ...
        #narrow_mask_file = "Y:\\lichao\\reg_write\\tonemask_narrow_b1_"+ NARROW_SOURCE +".txt"
        narrow_mask_file = "Z:\\lichao\\reg_write\\tonemask_narrow_b1_"+ NARROW_SOURCE +".txt"
        ###parameters ：tone_mask_file,ValidCarrierNum,band,narrow_f,afe_type
        tonemask_b1_3m = tonemask_narrow.ToneMaskConfig(narrow_mask_file,131,1,NARROW_SOURCE,config.DUT_AFE_TYPE)
        tonemask_b1_3m.run()
        
        #plc_tb_ctrl._debug('memwrite 32 0x5007001C 0x200020a0\r\n')
        
        #dut_sitrace.send_cli_cmd('memwrite 32 0x5007001c 0x200020a0\r\n')
        if tonemask_b1_3m.sync_ce_tm_enable:
            for cmd in tonemask_b1_3m.add_A000_static_para:
                dut_sitrace.send_cli_cmd(cmd)
                plc_tb_ctrl._debug(cmd)
            time.sleep(2)
            
            for cmd in tonemask_b1_3m.add_5007_static_para:
                dut_sitrace.send_cli_cmd(cmd)
                plc_tb_ctrl._debug(cmd)
            time.sleep(2) 
            
            for cmd in tonemask_b1_3m.add_5007_tonemask:
                dut_sitrace.send_cli_cmd(cmd)
                plc_tb_ctrl._debug(cmd)
            time.sleep(2)
            
        for cmd in tonemask_b1_3m.reg_narrow:
            dut_sitrace.send_cli_cmd(cmd)
            plc_tb_ctrl._debug(cmd)
        time.sleep(2)
