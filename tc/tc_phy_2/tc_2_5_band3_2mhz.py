...
def check(tdr, tb, test_data,result_file,siggen_plc):
    ...
    if test_narrow:
        ...
        narrow_mask_file = "Z:\\lichao\\reg_write\\tonemask_narrow_b3_"+ NARROW_SOURCE +".txt"
        ###parameters ：tone_mask_file,ValidCarrierNum,band,narrow_f,afe_type
        tonemask_b0 = tonemask_narrow.ToneMaskConfig(narrow_mask_file,49,3,NARROW_SOURCE,config.DUT_AFE_TYPE)
        tonemask_b0.run()
        
        #plc_tb_ctrl._debug('memwrite 32 0x5007001C 0x200020a0\r\n')
        
        #dut_sitrace.send_cli_cmd('memwrite 32 0x5007001c 0x200020a0\r\n')
        if tonemask_b0.sync_ce_tm_enable:
            for cmd in tonemask_b0.add_A000_static_para:
                dut_sitrace.send_cli_cmd(cmd)
                plc_tb_ctrl._debug(cmd)
            time.sleep(2)
            
            for cmd in tonemask_b0.add_5007_static_para:
                dut_sitrace.send_cli_cmd(cmd)
                plc_tb_ctrl._debug(cmd)
            time.sleep(2) 
            
            for cmd in tonemask_b0.add_5007_tonemask:
                dut_sitrace.send_cli_cmd(cmd)
                plc_tb_ctrl._debug(cmd)
            time.sleep(2)
            
        for cmd in tonemask_b0.reg_narrow:
            dut_sitrace.send_cli_cmd(cmd)
            plc_tb_ctrl._debug(cmd)
        time.sleep(2)
    ...
    while succ_rate < TARGET_SUCC_RATE:
        if total_cnt%100 == 0:  
            ...
            beacon_period_start_time = tb._configure_beacon(None, beacon_dict)
            # 等待5个信标都发送完成
            time.sleep(12*config.CLOCK_RATE)
