...
'''
1. 被测 CCO 上电；
2. 软件平台模拟集中器，向待测 CCO 下发“设置主节点地址”命令，在收到“确认”后，
向待测 CCO 下发“添加从节点”命令，将 STA 的 MAC 地址下发到 CCO 中，等待“确认”；
3. 软件平台接收到待测 CCO 发出的“中央信标报文”后，模拟第一个 STA 入网，发送“关
联请求报文”；
4. 软件平台收到待测 CCO 发出的“关联确认报文”之后，重复以上步骤模拟第二个 STA
入网；（以上步骤目的为了给待测 CCO 构造两个已入网的 STA 的情况，默认组网过程
正常，不作为检查项目）。
5. 软件平台模拟集中器，向待测 CCO 下发“集中器主动抄表”；
6. 软件平台收到待测 CCO 发出的“集中器主动抄表 SOF 帧”后，模拟第一个入网的 STA，
发送全网广播形式的“STA 抄表响应 SOF1” 报文，并设定 10s 的定时器；
7. 10s 定时器到时之前软件平台会收到待测 CCO 上报的响应内容，软件平台模拟第二个入
网的 STA，发出 SOF2 帧（是对 SOF1 帧的转发），并设定 10s 的定时器。
'''
def run(tb):
    """
    Args:
    ...
    tb._send_msdu(msdu=msdu, mac_head=tb.mac_head, src_tei=sta_tei_list[0], dst_tei=1,broadcast_flag = 1, sn = 1)
    ...
    tb._send_msdu(msdu=msdu, mac_head=tb.mac_head, src_tei=sta_tei_list[0], dst_tei=1,broadcast_flag = 1, sn = 2)
    #wait for meter read result coming from DUT(CCO)
    # 监控是否能够在n秒内收到CCO 上报的Q／GDW 1376.2 协议AFN13HF1 应答报文
    frame1376p2 = cct.wait_for_gdw1376p2_frame(afn=0x13, dt1=0x01, dt2=0, timeout=10)
    #监控集中器是否能收到待测CCO 转发的抄表上行报文。
    ...
    tb._send_msdu(msdu=msdu, mac_head=tb.mac_head, src_tei=sta_tei_list[0], dst_tei=1,broadcast_flag = 1, sn = 3)
    #wait for meter read result coming from DUT(CCO)
    # 监控是否能够在n秒内收到CCO 上报的Q／GDW 1376.2 协议AFN13HF1 应答报文
    frame1376p2 = cct.wait_for_gdw1376p2_frame(afn=0x13, dt1=0x01, dt2=0, timeout=10)
    #监控集中器是否能收到待测CCO 转发的抄表上行报文。
    ...
    tb._send_msdu(msdu=msdu, mac_head=tb.mac_head, src_tei=sta_tei_list[0], dst_tei=1,broadcast_flag = 1, sn = 4)
    #wait for meter read result coming from DUT(CCO)
    # 监控是否能够在n秒内收到CCO 上报的Q／GDW 1376.2 协议AFN13HF1 应答报文
    frame1376p2 = cct.wait_for_gdw1376p2_frame(afn=0x13, dt1=0x01, dt2=0, timeout=10)
    #监控集中器是否能收到待测CCO 转发的抄表上行报文。
    ...
    #tb._send_msdu(msdu=msdu, mac_head=tb.mac_head, src_tei=sta_tei_list[1], dst_tei=1,broadcast_flag = 1, sn = 2)
