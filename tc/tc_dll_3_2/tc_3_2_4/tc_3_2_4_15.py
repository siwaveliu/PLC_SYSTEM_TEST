...
'''
3.2.4.15 短 MPDU 帧载荷长度 264 短 MAC 帧分多包 MPDU 的 SOF 帧是否能够被正确处理测试
验证短 mpdu 帧载荷长度 264 短 mac 帧分多包 mpdu 的 sof 帧是否能够被正确处理
'''
def run(tb):
    """
    run function.
    ...
    pb_num_per_mpdu = 1
    total_pb_num = 4
    msdu_len = plc_packet_helper.calc_max_msdu_len(pb_size, total_pb_num, True)
