...
def run(tb):
    ...
    # 添加从节点地址
    frame = concentrator.build_gdw1376p2_frame(data_file='afn11f1_dl.yaml')
    ...
    while beacon_period_num < 10:
        ...
        assert beacon_payload.cco_mac_addr == cco_mac_addr, "{},{}".format(beacon_payload.cco_mac_addr, cco_mac_addr)
        ...
        if next_phase == 1:
            ...
            assert plc_packet_helper.check_ntb(last_beacon_period_start_time, beacon_fc.var_region_ver.timestamp,
                beacon_slot_len, beacon_slot_len * 2), "{},beacon_slot_len:{}".format(
                    plc_packet_helper.ntb_diff(last_beacon_period_start_time, beacon_fc.var_region_ver.timestamp), beacon_slot_len)
