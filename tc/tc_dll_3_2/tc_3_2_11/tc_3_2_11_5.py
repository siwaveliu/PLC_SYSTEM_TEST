...
7. 被测 sta 收到关联确认报文后。
8. 测试用例通过载波透明接入单元发送代理信标报文，代理信标中安排被测 sta 发现信
标时隙，路由周期为 20s，代理站点发现列表周期为 2s，发现站点发现列表周期为 2s。
9. 被测 sta 收到代理信标帧后，根据发现列表周期发送发现列表报文。
10. 测试用例通过载波透明接入单元按照代理站点发现列表周期发送发现列表报文。
11. 载波信道侦听单元收到被测 sta 的发现列表报文后，上传给测试台体，再送给一致性
...
#     [beacon_fc, beacon_payload] = tb._wait_for_plc_beacon(15)[1:]
    ...
    assert beacon_payload.beacon_info.beacon_item_list[0].beacon_item.level == 1 + 1
