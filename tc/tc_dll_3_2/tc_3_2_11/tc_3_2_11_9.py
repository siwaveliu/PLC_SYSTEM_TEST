...
4. 载波信道侦听单元收到被测试 sta 的关联请求报文后，上传给测试台体，再送给一致性评
价模块。
5. 一致性评价模块判断被测 sta 的关联请求报文正确后，通知测试用例。
6. 测试用例通过载波透明接入单元发送关联确认报文。
7. 被测 sta 收到关联确认报文后。
8. 测试用例通过载波透明接入单元发送中央信标报文，中央信标中安排被测 sta 发现信标时
隙。
9. 被测 sta 收到中央信标帧后，根据中央信标安排的时隙发送发现信标报文。
10. 载波信道侦听单元收到被测 sta 的发现信标报文后，上传给测试台体，再送给一致性评价
模块。
11. 测试用例变更组网序列号通过载波透明接入单元发送中央信标报文(组网序列号变化)。
12. 被测 sta 收到中央信标帧后，发现组网序列号发生变化，重新发起关联请求报文。
13. 载波信道侦听单元收到被测 sta 的关联请求报文后，上传给测试台体，再送给一致性评价
模块。
14. 一致性评价模块判断被测 sta 的关联请求报文正确后，通知测试用例。
15. 测试用例通过载波透明接入单元发送关联确认报文。
16. 被测 sta 收到关联确认报文后。
17. 测试用例通过载波透明接入单元发送中央信标报文，中央信标中安排被测 sta 发现信标时
隙。
18. 被测 sta 收到中央信标帧后，根据中央信标安排的时隙发送发现信标报文。
19. 载波信道侦听单元收到被测 sta 的发现信标报文后，上传给测试台体，再送给一致性评价
模块。
'''
def run(tb):
    """
...
#             route_period = beacon_item['beacon_item']['route_period']
    ...
    #modify network organizing SN
    beacon_dict = tb._load_data_file('tb_beacon_data.yaml')
    beacon_dict['payload']['value']['nw_sn'] += 1
    beacon_dict['payload']['value']['association_start'] = 1
    tb._configure_beacon(None, beacon_dict,True)
