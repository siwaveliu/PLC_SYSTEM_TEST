...
'''
3.2.5.9 STA 在发送单播 SOF 帧后，接收到对应的 SACK 帧能否正确处理测试
'''
def run(tb):
    """
    run function.
    ...
    # step12
    # 配置TB自动发送SACK, 第一次失败，第二次成功
    sack_data_list = [
        {"nid":nid, "rx_result":1, "rx_status":0, "dst_tei":sta_tei, "src_tei":1, "rx_pb_num": 1},
        {"nid":nid, "rx_result":0, "rx_status":1, "dst_tei":sta_tei, "src_tei":1, "rx_pb_num": 1},
    ]
    tb._config_sack_auto_reply_ex(1, sack_data_list)
    ...
    tb._send_msdu(msdu=msdu, mac_head=tb.mac_head, src_tei=1, dst_tei=sta_tei, auto_retrans=False)
    ...
    assert fc.var_region_ver.src_tei == sta_tei, "src tei is not 1"
    ...
    assert mac_frame_head.org_src_tei == sta_tei, "wrong org_src_tei"
