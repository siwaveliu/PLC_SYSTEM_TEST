...
def check_sof(tb, cct, nid, sta_tei, rx_result, rx_status):
    # step12
    # 配置TB自动发送SACK
    tb._config_sack_tei(nid, sta_tei, sta_tei, 1,
...
'''
3.2.5.4 CCO 在发送单播 SOF 帧后，接收到对应的 SACK 帧能否正确处理测试
验证 CCO 在发送单播 sof 帧后，接收到对应的 SACK 帧能否正确处理
'''
def run(tb):
    """
    run function.
    ...
    # 正确的SACK，CCO不重传
    check_sof(tb, cct, beacon_fc.nid, sta_tei, 0, 1)
    ...
    # 不正确的SACK，CCO重传
    check_sof(tb, cct, beacon_fc.nid, sta_tei, 1, 0)
