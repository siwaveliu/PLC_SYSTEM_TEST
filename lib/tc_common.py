...
def build_sof_fc_pl_data_ex(mac_frame, nid, src_tei, dst_tei, tmi_b=None, tmi_e=None, pb_num=None, ack_needed=True,
                            encrypt_flag=0,
                            lid=0, retrans_flag=0, broadcast_flag=0, pb_start_sn=0, total_pb_num=None,
    ...
    if (tmi_b is None) and (tmi_e is None):
        ...
        sof_var_region_ver = dict(lid=lid, dst_tei=dst_tei, src_tei=src_tei,
                                  pb_num=pb_format['pb_num'], frame_len=frame_len, tmi_b=pb_format['tmi_basic_mode'],
                                  encrypt_flag=encrypt_flag,
                                  retrans_flag=retrans_flag, broadcast_flag=broadcast_flag,
                                  symbol_num=pb_format['sym_num'],
...
# DUT == STA
def apl_sta_network_connect(tb_inst, m_inst, cco_mac, meter_addr, sta_tei, beacon_loss=0, beacon_proxy_flag=0,
                            need_tb_deactive=False):
    assert isinstance(tb_inst, plc_tb_ctrl.PlcSystemTestbench), "tb_inst type is not plc_tb_ctrl.PlcSystemTestbench"
...
def check_nw_top(cct, nw_top, timeout=1200):
    ...
    query_amount = 30
    while True:
        afn10f21_ul_frame = query_nw_top(cct, start_index, query_amount)
        ...
        start_index = start_index + query_amount
        if (start_index > data.total_num):
            start_index = 1
        ...
        time.sleep(out_nw_top.__len__() // 10 + 1)
    ...
    for i in range(num):
        ...
        for sta in sta_addr:
            for di in di_list:
                ...
                gdw1376p2_frame = cct.wait_for_gdw1376p2_frame(afn=0x13, dt1=0x01, dt2=0, timeout=timeout,
                                                               tm_assert=False)
...
def exec_cct_mr_multiple(tb, cct, cco_addr, sta_addr_list, timeout, num, di_list):
    ...
    for i in range(num):
        ...
        while len(sta_addr_list_copy) > 0:
            gdw1376p2_frame = cct.wait_for_gdw1376p2_frame(afn=0xF1, dt1=0x01, dt2=0, timeout=timeout_copy,
                                                           tm_assert=False)
...
def write_cco_band(cct, band):
    ...
    for i in range(3):
        ...
        cct.send_frame(frame)
        # 等待确认
        rcv = cct.wait_for_gdw1376p2_frame(afn=0x00, dt1=0x01, dt2=0, tm_assert=False)
        if rcv is None:
            cct.reset_port()
        else:
            break
...
def read_node_top_list(file, cco_mac_addr=None, log=False):
    ...
    if log:
        ...
        plc_tb_ctrl._debug(len(sec_nodes_addr_list))
