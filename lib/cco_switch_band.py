...
def run(tb, cct, band, addrfilepath, *channel):
    ...
    if  r_band != band:
        ...
        tc_common.check_nw_top(cct, nw_top_main, 3600 * 3)

        tc_common.write_cco_band(cct, band)
        plc_tb_ctrl._debug("wait for cco switch band, sleep 300s")
        ...
        #         break
        #     payload = tb._wait_for_plc_beacon(60, timeout_cb=timeout_function)
        #     if payload is not None:
        #         bc_payload = payload[2]
        #         if bc_payload.cco_mac_addr == cco_mac_addr or bc_payload.cco_mac_addr == "00-A1-93-21-86-91":
        #             wait = False
        #             break
        #         else:
